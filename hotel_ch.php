    ...
    function booking_emailapi(id){
        if(emailbind(id,$("#"+id).val())){
            $.ajax({
                ...
                type: 'POST',
                dataType: 'text',
                data: {email:$("#form3 #"+id_str3+"email").val()},
...
<script>
    ...
    function login_booking_room(OAuth_Id){
        ...
        $.ajax({
            url: 'booking_room_api.php',
            type: 'POST',
            dataType: 'text',
            data: {user_id:user_id,hotel_id:hotel_id,Checkin_date:Checkin_date,Checkout_date:Checkout_date,adult:adult,child:child,nights:nights,room:room},
            success:function(data){
                if(data){
                    alert('訂房成功');
                    location.reload();
    ...
    ?>
        login_booking_room("<?php echo $_SESSION['OAuth_Id'];?>");
    <?php
        }else{
    ?>  
        $.ajax({
...
<?php
	...
	$sql="SELECT * FROM hotel_ch WHERE hotel_Id LIKE '%".substr(base64_decode(str_replace(" ","+",$_GET['hotel_ch_Id'])), 1)."%'";
    ...
    if(isset($_SESSION['OAuth_Id'])){
        $sqlhotel="SELECT * FROM `room_order` WHERE `hotel_Id`='".$row['hotel_Id']."' AND `OAuth_Id`='".$_SESSION['OAuth_Id']."'"; 
    
        $bookingresault=sqldatabaselink($link,$sqlhotel);
...
<div class="card  text-center">
    ...
    <div class=" up-img card-img-overlay">
        ...
        <img class="avatar rounded-circle gradient-card-header purple-gradient img-thumbnail"  src="<?php echo $row['Picture1']!=""?$row['Picture1']:'img/bg.png';?>" onerror="this.src='img/bg.png'">
    </div>
