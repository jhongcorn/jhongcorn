<script>
  ...
  function forget(){         
    ...
    if(forget_email_flag && forget_phone_flag && forget_birthday_flag){
      ...
      $.ajax({
        url: 'forget_password_api.php',
        type: 'POST',
        dataType: 'json',
        data:{forget_email:$("#forget_email").val(),forget_phone:$("#forget_phone").val(),forget_birthday:$("#forget_birthday").val()},
        success:function(data){
          if(data){
            $("#modalLoginAvatar").modal("show");
            $("#forget_close").trigger("click");
            if(data[0]['picture']!=""){
               $("modalLoginAvatar .modal-header").append('<img src="'+data[0]['picture']+'" id="new_password_photo"  class="rounded-circle img-responsive">');
                ...
                $("#new_password_name").attr("customer_id",data[0]['Customer_Id']);
                $("#new_password_name").attr("oauth_id",data[0]['OAuth_Id']);
  ...
  function new_password_submit(){
    if(new_password_value_flag && re_new_password_value_flag){
      $.ajax({
        ...
        type: 'POST',
        dataType: 'text',
        data: {item:'userinfo_Customer_Password',value:$("#new_password_value").val(),id: $("#new_password_name").attr("customer_id"),OAuth_Id:$("#new_password_name").attr("oauth_id")},
        success:function(data){
  ...
  <div class="modal-dialog cascading-modal modal-avatar modal-sm" role="document">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <h5 class="mt-1 mb-2 text-center" id="new_password_name"></h5>
