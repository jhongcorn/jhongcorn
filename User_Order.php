...
<html lang="en">
	...
	<script>
		...
		function change_order_submit(){
			$.ajax({
                url: 'Change_Order_api.php',
                type: 'POST',
                dataType: 'text',
                data: {adult:$("#hotel_adult").val(),child:$("#hotel_child").val(),room:$("#hotel_room").val(),order_id:$("#Change_Order").attr("order-id"),hotel_id:$("#Change_Order").attr("hotel-id"),oauth_id:$("#Change_Order").attr("oauth-id"),customer_id:$("#Change_Order").attr("customer-id"),Change_Order:"<?php echo base64_encode('Change_Order');?>"},
		...
		function del_Order(id){
			...
			$.ajax({
                ...
                type: 'POST',
                dataType: 'text',
                data: {order_id:$('#'+card_id).attr("order-id"),hotel_id:$('#'+card_id).attr("hotel-id"),oauth_id:$('#'+card_id).attr("oauth-id"),customer_id:$('#'+card_id).attr("customer-id"),del_Order:"<?php echo base64_encode('del_Order');?>"},
                success:function(data){
 					if(data){
 						location.reload();
 					}else{
				...
				<div class="col-md-6 col-sm-12 my-3">
					<div class="card" id="order<?php echo $value['hotel_Id'].$row['Order_Id'];?>" adult="<?php echo $row['adult'];?>" child="<?php echo $row['child'];?>" room="<?php echo $row['room'];?>" order-id="<?php echo $row['Order_Id']?>" hotel-id="<?php echo $value['hotel_Id']?>" oauth-id="<?php echo $row['OAuth_Id']?>" customer-id="<?php echo $row['customer_Id']?>">
						...
						<div class="row text-center no-gutters">
							<div class="col-md-6 col-sm-12 "><img class="img-thumbnail card-img" style="height: 300px;width:300px" src="<?php echo $value['Picture1']!=""?$value['Picture1']:'img/bg.png';?>" onerror="this.src='img/bg.png'"></div>
							...
							<div class="card-footer col-12">
								<?php if(prDates($row['Checkin_date'], $row['Checkout_date'])>1 ){
									...
									echo '<a href="index.php?citylink='.base64_encode('citylink').'&hotel_ch_Id='.base64_encode($value['hotel_Id']).'&hotel_ch='.base64_encode('hotel_ch').'" class="card-link">再次預定</a>';
			...
			?>
				...
				<nav  aria-label="Page navigation example" class="d-flex">
				  <ul class="mr-auto ml-auto pagination">
				    ...
				    <li class="page-item"><a href="?pageNum=<?php echo base64_encode($pageNum-1);?>  "class="page-link"><i class="fas fa-angle-left text-info"></i></a></li><?php }?>
				    <li class="page-item"><span><?php echo "第".' '.($pageNum+1).' '.'頁';?></span></li>
				    <li class="page-item"><span><?php echo "/共".' '.($totalPages+1).' '."頁";?></span></li>
				    <?php if ($pageNum < $totalPages) {?>
				    <li class="page-item">
				      <a href="?pageNum=<?php echo base64_encode($pageNum+1);?> "class="page-link"><i class="fas fa-angle-right text-info"></i></a>
				    </li>
